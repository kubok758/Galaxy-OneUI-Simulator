...
type MailItem = { id: string; folder: MailFolder; from: string; to: string; subject: string; body: string; time: string; read: boolean; starred: boolean; attachment?: Attachment };
...
export function EmailApp() {
  ...
  const resetComposer = () => { setTo(''); setSubject(''); setBody(''); setAttachment(null); };
  const openMail = (id: string) => { setSelectedId(id); setMails((current) => current.map((mail) => mail.id === id ? { ...mail, read: true } : mail)); };
  const saveDraft = () => {
    if (!to.trim() && !subject.trim() && !body.trim() && !attachment) return;
    setMails((current) => [{ id: makeId('mail'), folder: 'drafts', from: 'artem@example.com', to: to.trim(), subject: subject.trim() || 'Без темы', body: body.trim(), time: 'Сейчас', read: true, starred: false, attachment: attachment ?? undefined }, ...current]);
  };
  const closeCompose = () => { saveDraft(); setComposeOpen(false); resetComposer(); };
  const sendMail = () => {
    if (!to.trim() || !body.trim()) return;
    setMails((current) => [{ id: makeId('mail'), folder: 'sent', from: 'artem@example.com', to: to.trim(), subject: subject.trim() || 'Без темы', body: body.trim(), time: new Date().toLocaleTimeString('ru-RU', { hour: '2-digit', minute: '2-digit' }), read: true, starred: false, attachment: attachment ?? undefined }, ...current]);
    setComposeOpen(false); resetComposer(); setFolder('sent');
  };
